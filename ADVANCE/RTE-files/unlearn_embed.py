...
UNLEARNED_MODEL_PATH = os.path.join(BASE_DIR, "models_rte", "advance_unlearned_embed_rerun_01.pth")  # v4
CHECKPOINT_PATH      = os.path.join(BASE_DIR, "models_rte", "advance_unlearned_alpha_02_ckpt.pth")
...
def compute_unlearning_loss(model_ori, model_unlearn,
    ...
    loss_ukr = 0.5 * (
        mse(out_df_un["img_emb"], out_df_ori["img_emb"]) +
...
print(f"\n{'='*60}")
print(f"  Runtime (unlearn_embed.py): {_h:02d}h {_m:02d}m {_s:02d}s  ({_elapsed:.1f}s total)")
print(f"{'='*60}")
