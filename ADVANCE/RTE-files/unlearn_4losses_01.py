...
TRAINED_MODEL_PATH   = os.path.join(BASE_DIR, "models_rte", "advance_trained_rerun_01.pth")
UNLEARNED_MODEL_PATH = os.path.join(BASE_DIR, "models_rte", "advance_unlearned_4loss_01_rerun.pth")
CHECKPOINT_PATH      = os.path.join(BASE_DIR, "models_rte", "advance_chk_ablation.pth")
...
def compute_unlearning_loss(model_ori, model_unlearn,
    ...
    teacher_mkr  = func_max_retain(out_dr_ori, retain_labels)
    loss_mkr     = MKR_loss(teacher_mkr, out_dr_un["fusion_logits"], 2.0, forget_labels, reduction="batchmean")

    # ── Loss MKR_UNI: image + audio on retain (T=2.0, dynamic a4) ────────────
    loss_mkr_uni = (
        MKR_loss(teacher_mkr, out_dr_un["audio_logits"], 2.0, forget_labels, reduction="batchmean") +
        MKR_loss(teacher_mkr, out_dr_un["image_logits"], 2.0, forget_labels, reduction="batchmean")
    ) / 2.0

    # ── Loss UKR: image + audio on forget (T=1.0, dynamic a3) ────────────────
    teacher_ukr   = func_max(out_df_ori, forget_labels)
    loss_ukr_image = UKR_loss(teacher_ukr, out_df_un["image_logits"], 1.0, reduction="batchmean")
