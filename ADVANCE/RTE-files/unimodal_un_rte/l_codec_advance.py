"""
L-CODEC (Conditional Independence) Unlearning for ADVANCE
==========================================================
...
#from src.model import AdvanceMultimodalModel, load_model
...
def compute_diagonal_fisher(model, loader, device, desc=""):
    ...
    for batch in tqdm(loader, desc=f"  Computing Fisher [{desc}]"):
        ...
        loss = (criterion(out['audio_logits'], labels) +
                criterion(out['image_logits'], labels) +
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                fisher[name] += param.grad.data.pow(2).to(device) * imgs.size(0)
...
def run_L_CODEC_unlearning(original_ckpt, save_ckpt_path, forget_class,
    ...
    """
    Run L-CODEC unlearning on ADVANCE dataset.
    ...
    # Get full datasets for Fisher computation
...
if __name__ == "__main__":
    ...
    ORIGINAL_CKPT = "/home/team2/Unlearning/ADVANCE/models_rte/advance_trained_rerun_01.pth"
    SAVE_CKPT_PATH = "/home/team2/Unlearning/ADVANCE/models_rte/unimodal_unlearn/advance_unlearned_lcodec.pth"
    ...
    print(f"\n{'='*60}")
    print(f"  Runtime (l_codec_advance.py): {_h:02d}h {_m:02d}m {_s:02d}s  ({_elapsed:.1f}s total)")
    print(f"{'='*60}")