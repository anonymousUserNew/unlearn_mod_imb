...
# a3  → UKR loss   — image + audio only   → input_dim = 3*C × 2 branches = 6C

net_a1 = WeightNet(input_dim=3 * C).to(DEVICE)     # MD  → fusion
net_a2 = WeightNet(input_dim=9 * C).to(DEVICE)     # MKR → fusion + image + audio
net_a3 = WeightNet(input_dim=6 * C).to(DEVICE)     # UKR → image + audio
...
def compute_dynamic_alphas(net_a1, net_a2, net_a3,
    ...
    vec_mkr = torch.cat([vec_mkr_fusion, vec_mkr_image, vec_mkr_audio], dim=1)  # (B, 9C)
    raw_a2 = net_a2(vec_mkr).mean()                # scalar

    # ── a3: UKR loss — image + audio branches, forget data ───────────────────
    # UKR target: func_max(out_df_ori, forget_labels)
    teacher_ukr = func_max(out_df_ori, forget_labels)
    vec_ukr_image = get_diff_vectors(out_df_un["image_logits"], teacher_ukr, gt_one_hot_forget)
...
def compute_unlearning_loss(model_ori, model_unlearn,
    ...
    total_loss = (a1 * loss_md) + (a2 * loss_mkr) + (a3 * loss_ukr) + loss_uni_ce
