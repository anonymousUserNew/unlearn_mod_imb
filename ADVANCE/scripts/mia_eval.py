...
def MIA(retain_loader_train, retain_loader_test, forget_loader, test_loader, model, device):
    """
    Standard MIA setup for unlearning evaluation:
      shadow_train  = retain_train   (model was trained on this → members)
      shadow_test   = test_loader    (model never saw this → non-members, all classes present)
      target_train  = retain_train   (same distribution as shadow train)
      target_test   = forget_loader  (the forget set — we want to check if model still remembers it)
    """
    ...
    target_train_performance = collect_performance(retain_loader_train, model, device)
...
def main():
    ...
    retain_loader_train = DataLoader(retain_train, batch_size=BATCH_SIZE, shuffle=False, num_workers=4)
    retain_loader_test  = DataLoader(retain_test,  batch_size=BATCH_SIZE, shuffle=False, num_workers=4)
    forget_loader       = DataLoader(forget_train, batch_size=BATCH_SIZE, shuffle=False, num_workers=4)
    # test_loader = general held-out set with ALL classes (retain_test serves this role)
    test_loader         = retain_loader_test
    ...
    for name, path in models_to_eval.items():
        print(f"\n--- Evaluating Metric (MIA) for model: {name} ---")
        try:
            model.load_state_dict(torch.load(path, map_location=DEVICE))
            model.eval()
            print("Model loaded successfully. Computing MIA...")
            MIA(retain_loader_train, retain_loader_test, forget_loader, test_loader, model, DEVICE)
