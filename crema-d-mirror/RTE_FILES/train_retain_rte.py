"""
Train CREMA-D Dual-ResNet50 Multimodal Model - Retain Split Only
================================================================
Trains the CremaMultimodalModel from scratch ONLY on the retain classes.
...
from src.labels  import NUM_CLASSES, CREMA_CLASSES
...
MODEL_PATH = os.path.join(BASE_DIR, "models/rte", "crema_trained_retain_only.pth")
...
def main():
    ...
    # We load ONLY the retain splits by filtering out the forget class 
    # from the original dataset's Train/Val/Test setup.
    train_ds, val_ds, test_ds = get_retain_splits(forget_class=FORGET_CLASS, seed=SEED)
    
    # Note: the full dataset size is reduced since we drop all 'FORGET_CLASS' samples
    ...
    # ── Model ──────────────────────────────────────────────────────────────────
    # Note: even though we're training only on 5 classes, we instantiate the 
    # model with all 6 classes (NUM_CLASSES) so that its architecture exactly
    # matches the unlearned model. It simply won't predict the forget class.
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch in loop:
            ...
            # Even though there are 6 classes in the logits space, the ground truth
            # labels will never be FORGET_CLASS, so it's naturally trained to avoid it.
            loss = (
                F.cross_entropy(out["fusion_logits"], labels) +
                F.cross_entropy(out["video_logits"],  labels) +
                F.cross_entropy(out["audio_logits"],  labels)
