...
UNLEARNED_MODEL_PATH = os.path.join(BASE_DIR, "models", "crema_unlearned.pth")  # v4
...
net_a1 = WeightNet(input_dim=3 * C).to(DEVICE)     # MKR fusion
net_a2 = WeightNet(input_dim=6 * C).to(DEVICE)     # UK forget → video + audio
net_a3 = WeightNet(input_dim=6 * C).to(DEVICE)     # UK retain → video + audio
...
def compute_dynamic_alphas(net_a1, net_a2, net_a3,
    ...
    vec_mkr_uni = torch.cat([vec_mkr_video, vec_mkr_audio], dim=1)  # (B, 9C)
    raw_a1 = net_a1(vec_mkr_fusion).mean()                # scalar
...
def compute_unlearning_loss(model_ori, model_unlearn,
    ...
    loss_mkr_uni = (
        MKR_loss(func_max_retain(out_dr_ori, retain_labels), out_dr_un["video_logits"],  2.0, forget_labels, reduction="batchmean") +
        MKR_loss(func_max_retain(out_dr_ori, retain_labels), out_dr_un["audio_logits"],  2.0, forget_labels, reduction="batchmean")
    ) / 2.0

    loss_mkr=MKR_loss(func_max_retain(out_dr_ori, retain_labels), out_dr_un["fusion_logits"], 2.0, forget_labels, reduction="batchmean")
    ...
    total_loss = (loss_md) + (a1 * loss_mkr) + (a2 * loss_ukr) + a3*loss_mkr_uni + 0.8*loss_uni_ce
...
for epoch in range(EPOCHS):
    ...
    if last_out is not None:
        print(
            ...
            f"a2={last_out['a2']:.3f}  a3={last_out['a3']:.3f}"
        )
    else:
