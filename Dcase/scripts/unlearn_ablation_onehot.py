...
UNLEARNED_MODEL_PATH = os.path.join(BASE_DIR, "models", "dcase_unlearned_ablation_onehot.pth")
CHECKPOINT_PATH      = os.path.join(BASE_DIR, "models", "dcase_chk_ablation_onehot.pth")
...
def compute_dynamic_alphas(net_a1, net_a2, net_a3,net_a4,
    ...
    vec_mkr_fusion = get_diff_vectors(out_dr_un["fusion_logits"], gt_one_hot_retain, gt_one_hot_retain)
    vec_mkr_audio = get_diff_vectors(out_dr_un["audio_logits"], gt_one_hot_retain, gt_one_hot_retain)
    vec_mkr_video = get_diff_vectors(out_dr_un["video_logits"], gt_one_hot_retain, gt_one_hot_retain)
    ...
    vec_ukr_video = get_diff_vectors(gt_one_hot_forget, out_df_un["video_logits"], gt_one_hot_forget)
    vec_ukr_audio = get_diff_vectors(gt_one_hot_forget, out_df_un["audio_logits"], gt_one_hot_forget)
...
def compute_unlearning_loss(model_ori, model_unlearn,
    ...
    teacher_mkr = func_max_retain(out_dr_ori, retain_labels)
    loss_mkr_fusion = MKR_loss(gt_one_hot_retain, out_dr_un["fusion_logits"], T, forget_labels, reduction="batchmean")
    loss_mkr_audio = MKR_loss(gt_one_hot_retain, out_dr_un["audio_logits"], T, forget_labels, reduction="batchmean")
    loss_mkr_video = MKR_loss(gt_one_hot_retain, out_dr_un["video_logits"], T, forget_labels, reduction="batchmean")
    ...
    teacher_ukr = func_max(out_df_ori, forget_labels)
    loss_ukr_video = UKR_loss(gt_one_hot_forget, out_df_un["video_logits"], T, reduction="batchmean")
    loss_ukr_audio = UKR_loss(gt_one_hot_forget, out_df_un["audio_logits"], T, reduction="batchmean")
