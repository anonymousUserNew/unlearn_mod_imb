...
MODEL_PATH = "/home/team2/Unlearning/Dcase/models/dcase_unlearned_ablation_onehot.pth"

# Which data split to evaluate on:
#   "val"    → pre-split validation set
#   "test"   → pre-split test set (preferred for final evaluation)
#   "full"   → entire dataset
#   "forget" → only the forget class test split
#   "retain" → all classes except the forget class — test split
#   "train"  → training split (sanity/overfit check only)

DATA_SPLIT = "forget"

# Where to save outputs:
OUTPUT_DIR = "/home/team2/Unlearning/Dcase/outputs/unlearned_ablation_onehot/forget"
...
if DATA_SPLIT in ["train", "val", "test"]:
    ...
    print(f"  (Using pre-split {DATA_SPLIT} split)")
...
def plot_confusion_matrix(cm, classes, title, save_path):
    ...
    plt.xticks(ticks, classes, rotation=45, ha="right", fontsize=9)
    plt.yticks(ticks, classes, fontsize=9)
