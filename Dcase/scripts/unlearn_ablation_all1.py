...
UNLEARNED_MODEL_PATH = os.path.join(BASE_DIR, "models", "dcase_unlearned_ablations_all1.pth")
...
def compute_unlearning_loss(model_ori, model_unlearn,
    ...
    teacher_mkr = func_max_retain(out_dr_ori, retain_labels)
    loss_mkr_fusion = MKR_loss(teacher_mkr, out_dr_un["fusion_logits"], 2.0, forget_labels, reduction="batchmean")
    loss_mkr_audio = MKR_loss(teacher_mkr, out_dr_un["audio_logits"], 2.0, forget_labels, reduction="batchmean")
    loss_mkr_video = MKR_loss(teacher_mkr, out_dr_un["video_logits"], 2.0, forget_labels, reduction="batchmean")
    ...
    teacher_ukr = func_max(out_df_ori, forget_labels)
    loss_ukr_video = UKR_loss(teacher_ukr, out_df_un["video_logits"], 1.0, reduction="batchmean")
    loss_ukr_audio = UKR_loss(teacher_ukr, out_df_un["audio_logits"], 1.0, reduction="batchmean")
    ...
    total_loss = (loss_md) + ((loss_mkr)) + (loss_ukr) + (loss_mkr_uni) + ukr_loss_ce
