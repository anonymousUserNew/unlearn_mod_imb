...
UNLEARNED_MODEL_PATH = os.path.join(BASE_DIR, "models", "dcase_unlearned_ablation_imbalance.pth")
CHECKPOINT_PATH      = os.path.join(BASE_DIR, "models", "dcase_chk_ablation_imbalance.pth")
...
net_a1 = WeightNet(input_dim=3 * C).to(DEVICE)     # MD  → forget data
net_a2 = WeightNet(input_dim=3 * C).to(DEVICE)     # MKR → all three branches on retain
net_a3 = WeightNet(input_dim=6 * C).to(DEVICE)     # UKR → best branch pair on forget
net_a4 = WeightNet(input_dim=6 * C).to(DEVICE)     # MKR_UNI → best branch pair on forget
...
def func_max(out, forget_labels):
    """Select best branch (audio or video) based on forget class probability"""
...
def func_max_retain(out, retain_labels):
    """Select best branch (fusion, audio, or video) based on retain class probability"""
    all_logits = torch.stack(
...
def compute_dynamic_alphas(net_a1, net_a2, net_a3,net_a4,
    ...
    # ── a2: MKR loss — All three branches on retain data ──────────────────────
    teacher_mkr = func_max_retain(out_dr_ori, retain_labels)
    
    vec_mkr_fusion = get_diff_vectors(out_dr_un["fusion_logits"], out_dr_ori["fusion_logits"], gt_one_hot_retain)
    vec_mkr_audio = get_diff_vectors(out_dr_un["audio_logits"], out_dr_ori["audio_logits"], gt_one_hot_retain)
    vec_mkr_video = get_diff_vectors(out_dr_un["video_logits"], out_dr_ori["video_logits"], gt_one_hot_retain)

    vec_mkr_uni = torch.cat([vec_mkr_audio, vec_mkr_video], dim=1)  # (B, 9C)
    raw_a2 = net_a2(vec_mkr_fusion).mean()
    raw_a4 = net_a4(vec_mkr_uni).mean()

    # ── a3: UKR loss — Best branch pair on forget data ──────────────────────
    teacher_ukr = func_max(out_df_ori, forget_labels)
    
    vec_ukr_video = get_diff_vectors(out_df_ori["video_logits"], out_df_un["video_logits"], gt_one_hot_forget)
    vec_ukr_audio = get_diff_vectors(out_df_ori["audio_logits"], out_df_un["audio_logits"], gt_one_hot_forget)
    vec_ukr = torch.cat([vec_ukr_video, vec_ukr_audio], dim=1)  # (B, 6C)
    raw_a3 = net_a3(vec_ukr).mean()

    # ── Normalise ─────────────────────────────────────────────────────────────
    scores = torch.stack([raw_a2, raw_a3,raw_a4])    # (3,)
    weights = F.softmax(scores, dim=0) * 3             # sums to 3
    # a1_final=a1_base + F.tanh(raw_a1)
    # a2_final=a2_base + F.tanh(raw_a2)
    # a3_final=a3_base + F.tanh(raw_a3)

    # # scores = torch.stack([a1_final, a2_final, a3_final])
    # # weights = F.softmax(scores, dim=0) * 3
    return weights[0],weights[1],weights[2]

    # return a1_final,a2_final,a3_final
...
def compute_unlearning_loss(model_ori, model_unlearn,
    ...
    # ── Loss MD: Forget fusion with uniform distribution ─────────────────────
    ...
    #loss_md = MD_loss(out_df_ori["fusion_logits"], out_df_un["fusion_logits"], T, forget_labels, reduction="batchmean") 
    # ── Loss MKR: Protect all three branches on retain set ──────────────────
    teacher_mkr = func_max_retain(out_dr_ori, retain_labels)
    loss_mkr_fusion = MKR_loss(out_dr_ori["fusion_logits"], out_dr_un["fusion_logits"], 2.0, forget_labels, reduction="batchmean")
    loss_mkr_audio = MKR_loss(out_dr_ori["audio_logits"], out_dr_un["audio_logits"], 2.0, forget_labels, reduction="batchmean")
    loss_mkr_video = MKR_loss(out_dr_ori["video_logits"], out_dr_un["video_logits"], 2.0, forget_labels, reduction="batchmean")
    loss_mkr = (loss_mkr_fusion)

    loss_mkr_uni = (loss_mkr_audio + loss_mkr_video)/2.0

    # ── Loss UKR: Cross-modal on forget using best branches ────────────────
    teacher_ukr = func_max(out_df_ori, forget_labels)
    loss_ukr_video = UKR_loss(out_df_ori["video_logits"], out_df_un["video_logits"], 1.0, reduction="batchmean")
    loss_ukr_audio = UKR_loss(out_df_ori["audio_logits"], out_df_un["audio_logits"], 1.0, reduction="batchmean")
    loss_ukr = (loss_ukr_video + loss_ukr_audio)/2.0

    mkr_loss_ce = (F.cross_entropy(out_dr_un["video_logits"], retain_labels)+F.cross_entropy(out_dr_un["audio_logits"], retain_labels))
    ukr_loss_ce = (F.cross_entropy(out_df_un["video_logits"], forget_labels)+F.cross_entropy(out_df_un["audio_logits"], forget_labels))
    total_loss = (loss_md) + (a2*(loss_mkr)) + (a3*loss_ukr) + (a4*loss_mkr_uni)
    
    # B, C_dim = out_df_un["fusion_logits"].shape
    # # Build uniform target: 1/(C-1) for all classes except the forget class
    # target = torch.ones(B, C_dim, device=out_df_un["fusion_logits"].device)
    # target[torch.arange(B), forget_labels] = 0.0
    # target = target / target.sum(dim=1, keepdim=True)
    # total_loss = F.cross_entropy(out_df_un["fusion_logits"], target)+F.cross_entropy(out_dr_un["fusion_logits"], retain_labels)+F.cross_entropy(out_df_un["video_logits"], forget_labels)+F.cross_entropy(out_df_un["audio_logits"], forget_labels)+F.cross_entropy(out_dr_un["video_logits"], retain_labels)+F.cross_entropy(out_dr_un["audio_logits"], retain_labels)
...
for epoch in range(EPOCHS):
    ...
    # val_acc_df = {"fusion": 0.0, "video": 0.0, "audio": 0.0}
    # val_acc_dr = {"fusion": 0.0, "video": 0.0, "audio": 0.0}
