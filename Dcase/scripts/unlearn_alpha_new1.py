...
UNLEARNED_MODEL_PATH = os.path.join(BASE_DIR, "models", "dcase_unlearned_model.pth")
...
net_a2 = WeightNet(input_dim=9 * C).to(DEVICE)     # MKR → all three branches on retain
...
def compute_dynamic_alphas(net_a1, net_a2, net_a3,
                            out_df_un, out_dr_un,
    ...
    vec_mkr = torch.cat([vec_mkr_fusion,vec_mkr_audio, vec_mkr_video], dim=1)  # (B, 9C)
...
def compute_unlearning_loss(model_ori, model_unlearn,
    ...
    a1,a2, a3 = compute_dynamic_alphas(
        net_a1, net_a2, net_a3,
        out_df_un, out_dr_un,
        out_df_ori, out_dr_ori,out_ori_random,
        forget_labels, retain_labels,
    ...
    teacher_mkr = func_max_retain(out_dr_ori, retain_labels)
    loss_mkr_fusion = MKR_loss(teacher_mkr, out_dr_un["fusion_logits"], T, forget_labels, reduction="batchmean")
    loss_mkr_audio = MKR_loss(teacher_mkr, out_dr_un["audio_logits"], T, forget_labels, reduction="batchmean")
    loss_mkr_video = MKR_loss(teacher_mkr, out_dr_un["video_logits"], T, forget_labels, reduction="batchmean")
    loss_mkr = (loss_mkr_fusion+loss_mkr_audio+loss_mkr_video)/3.0

    # loss_mkr_uni = (loss_mkr_audio + loss_mkr_video)/2.0

    # ── Loss UKR: Cross-modal on forget using best branches ────────────────
    teacher_ukr = func_max(out_df_ori, forget_labels)
    loss_ukr_video = UKR_loss(teacher_ukr, out_df_un["video_logits"], T, reduction="batchmean")
    loss_ukr_audio = UKR_loss(teacher_ukr, out_df_un["audio_logits"], T, reduction="batchmean")
    ...
    total_loss = (a1*loss_md) + (a2*(loss_mkr)) + (a3*loss_ukr) 
...
for epoch in range(EPOCHS):
    model_unlearn.train()
    net_a1.train(); net_a2.train(); net_a3.train()
    ...
    model_unlearn.eval()
    net_a1.eval(); net_a2.eval(); net_a3.eval()
    ...
    if last_out is not None:
        print(
            ...
            f"a1={last_out['a1']:.3f} a2={last_out['a2']:.3f} a3={last_out['a3']:.3f}"
        )
    else:
